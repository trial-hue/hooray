...
import { cardCost, CONSUMER, gbp, pence, UNIT } from "@/lib/costs";
            ...
            <aside className="grid min-w-0 content-start gap-3 text-sm">
              <details className="card-panel" open>
                ...
                <div className="px-4 pb-4">
                  ...
                  <p className="mt-3 font-display text-2xl">{personal ? gbp(CONSUMER.perCardGbp) : gbp(cost.total)}</p>
                  <p className="hint text-xs">{personal ? `a card including post, or free on Circle at ${gbp(CONSUMER.subscriptionYearlyGbp)} a year · costs ${gbp(cost.print)} to print and post` : `${gbp(cost.print)} print, envelope and post via Docmail · AI ${pence(cost.ai)}`}</p>
                </div>
              </details>
