...
export function parseRoster(csv: string): ImportResult {
  ...
  for (const r of parsed.data) {
    ...
    const p: Person = {
      ...
      birthday: normaliseBirthday(r.birthday || r.dob, warnings, `${first} ${last}`),
      dob: /^\d{4}-\d{2}-\d{2}$/.test(r.dob?.trim() ?? "") ? r.dob.trim() : undefined,
      endDate: /^\d{4}-\d{2}-\d{2}$/.test(r.end_date?.trim() ?? "") ? r.end_date.trim() : undefined,
      retiring: /^(true|yes|1)$/i.test(r.retiring?.trim() ?? "") || undefined,
      ...
      consentOccasions: !/^(false|no|0)$/i.test(r.consent?.trim() ?? ""),
      deliverTo,
...
export function toCsv(people: Person[], byId: Map<string, Person>): string {
  const rows = people.map((p) => {
    ...
    const row: Row = {
      ...
      birthday: p.dob ? "" : (p.birthday ?? ""),
      dob: p.dob ?? "",
      end_date: p.endDate ?? "",
      retiring: p.retiring ? "true" : "",
